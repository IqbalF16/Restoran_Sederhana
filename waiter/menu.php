...
if($_SESSION['level']!='waiter'){
		die('Anda bukan waiter');
	}
?>

<h3><span class="glyphicon glyphicon-briefcase"></span>  Data Menu</h3>
...
<table class="table table-hover">
	...
	if(isset($_GET['cari'])){
		...
		$brg=mysql_query("SELECT * from menu where nama_menu like '%$cari%'");
	}else{
		$brg=mysql_query("SELECT * from menu");
	}
	$no=1;
	while($b=mysql_fetch_array($brg)){
		...
		<tr>
			...
			<td>
				<a href="edit_menu.php?id=<?php echo $b['id']; ?>" class="btn btn-warning">Edit</a>
				<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='hapus_menu.php?id=<?php echo $b['id']; ?>' }" class="btn btn-danger">Hapus</a>
			</td>
		</tr>		
		<?php 
	}
			...
			<div class="modal-body">
				<form action="tmb_brg_act.php" method="post">
					<div class="form-group">
						...
						<input name="nama_menu" type="text" class="form-control" placeholder="Nama Menu ..">
					</div>
					<div class="form-group">
						<label>Harga</label>
